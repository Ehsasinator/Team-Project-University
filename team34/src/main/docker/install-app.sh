...
sed -i "5s|latest|main${CI_COMMIT_TAG}latest|" ~/team-project/app.yml
sed -i "21s|project-ai-model|${CI_REGISTRY_IMAGE}:ai-model${CI_COMMIT_TAG}latest|" ~/team-project/app.yml
docker compose -f ~/team-project/app.yml pull
...
if [ -n "${CI_COMMIT_TAG}" ]; then
    ...
    sed -i "5s|latest|main${CI_COMMIT_TAG}latest|" ~/team-project/prd.yml
    sed -i "19s|project-ai-model|${CI_REGISTRY_IMAGE}:ai-model${CI_COMMIT_TAG}latest|" ~/team-project/prd.yml
    docker compose -f ~/team-project/prd.yml pull
