...
type RestOf<T extends IReport | NewReport> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class ReportService {
  ...
  protected convertDateFromServer(restReport: RestReport): IReport {
    return {
      ...restReport,
      date: restReport.date ? dayjs(restReport.date) : undefined,
    };
  }
  ...
  deleteEntry(entryId: number): Observable<HttpResponse<{}>> | null {
    //turn entryId into String
    let entryIdString = entryId.toString();

    //create the apiUrl
    let apiUrl = `${this.applicationConfigService.getEndpointFor('api/entries')}/${entryIdString}`;
